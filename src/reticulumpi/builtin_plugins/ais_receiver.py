...
class AISReceiver(SignalPluginBase):
    ...
    def _on_decoder_started(
        ...
        if self.plugin_state.value == "ready":
            self.mark_ready()
        self._start_stderr_reader(process, prefix="ais")
        self._start_thread(lambda: self._parser_loop(process), name="ais-parser")
        if was_retry:
            self.log.warning(
                "AIS receiver restarted after unexpected exit (attempt %d)",
                self._restart_count,
            )
    ...
    def _on_decoder_exhausted(self, failure: ProcessFailure) -> None:
        self._status = "error"
        self._last_error = f"AIS restart budget exhausted: {failure.reason}"
        self.mark_degraded(self._last_error)
    ...
    def _parser_loop(self, process: subprocess.Popen[Any] | None = None) -> None:
        ...
        try:
            ...
            if self._active and self._process is proc and group is not None and group.running:
                group.notify_unexpected_eof(0, "AIS decoder stdout ended")
